   ...
   public class EventoRepository : Repository<Evento>, IEventoRepository
    {
        ...
        public override Evento ObterPorId(Guid id)
        {
            ...
            var sql = @"SELECT * FROM Eventos E " +
                  ...
                  "ON E.Id = EN.EventoId " +
                  "WHERE E.Id = @uid"; //evitar SQL Injection

            var evento = Db.Database.GetDbConnection().Query<Evento, Endereco, Evento>(sql,
                (e, en) => //e = Evento // en = Endereco
                {
                    if (en != null) //pq e LEFT JOIN
                        e.AtribuirEndereco(en); //porque o set e privado

                    return e;
                }, new { uid = id }); //atribuindo o parametro do metodo ao parametro da Query
        ...
        {
            //override apenas para garantor que o metodo nao sera chamado para exclusao. 
            //Logica ja esta sendo feita no EventoCommandHandler: method: Handle(ExcluirEventoCommand message)
